...
-------------------------------------------------------------------------
output : OK(0=失敗), status(1=有欄位未填 / 2=有欄位格式不符 / 3=縣市跟櫃點對不起來 / 4=己額滿 / 5=手機己登記,且仍在可領取時間內 / 6=己兌換 / 7=不明錯誤)
...
//兌換頁活動網址
$exchange_url="https://www.shiseido-event.com/2017bioperformance/exchange.html";
		...
		if ($json==null) {
			switch ($func) {
				case "CityStoreListByEvent":
					$sqlStr='SELECT C.* FROM City C, Store S WHERE c_no=s_c_no AND s_e_no=:event_no AND s_req_total>0 AND (s_req_total-s_req_apply)>0 AND s_show=:event_no Group by c_name ORDER BY c_no ASC';
					$db->query($sqlStr);
					$db->bind(':event_no', $event_no, PDO::PARAM_INT);
					$cityRows = $db->resultset();

					$sqlStr='SELECT c_no, c_name, s_no, s_code, s_name, s_req_total, (s_req_total-s_req_apply) AS s_req_avail FROM Store, City WHERE c_no=s_c_no AND s_e_no=:event_no AND s_req_total>0 AND (s_req_total-s_req_apply)>0 AND s_show=1 order by s_order ASC';
					$db->query($sqlStr);
					$db->bind(':event_no', $event_no, PDO::PARAM_INT);
					...
					break;
				case "SubmitForm":
					if (isset($_GET['c_no']) && isset($_GET['s_no']) && isset($_GET['t_name']) && isset($_GET['t_mobile'])) {
						$arrInputFields=array('c_no'=>array('int'), 's_no'=>array('int'), 't_name'=>array('string'), 't_mobile'=>array('string',$mobileFormat));
						$c_no=$common->replaceParameter($_GET['c_no']);
						$s_no=$common->replaceParameter($_GET['s_no']);
						...
						if ($return_OK==0) {
							...
							//新增登登記者
							$sqlStr="INSERT INTO Tester(t_e_no, t_c_no, t_s_no, t_name, t_mobile, t_apply_ip) VALUES(:event_no, :c_no, :s_no, :t_name, :t_mobile, :t_apply_ip)";
							$db->query($sqlStr);
							$db->bind(':event_no', $event_no, PDO::PARAM_INT);
							$db->bind(':c_no', $c_no, PDO::PARAM_INT);
							$db->bind(':s_no', $s_no, PDO::PARAM_INT);
							...
							$json=array('OK'=>'1', 'status'=>$return_status, 't_no'=>$t_no, 'token'=>$accesstoken);
						}
					}
					break;
				case "SentMms":
					if (isset($_GET['c_no']) && isset($_GET['s_no']) && isset($_GET['t_no']) && isset($_GET['token'])) {
						...
						//檢查資料庫 ==> City & Store
						if ($return_OK==1) {
							$sqlStr="SELECT COUNT(s_no) FROM Store WHERE s_c_no=:c_no AND s_no=:s_no AND s_e_no=:event_no";
							$db->query($sqlStr);
							$db->bind(':c_no', $c_no, PDO::PARAM_INT);
							$db->bind(':s_no', $s_no, PDO::PARAM_INT);
							$db->bind(':event_no', $event_no, PDO::PARAM_INT);
						...
						//檢查資料庫 ==> Tester
						if ($return_OK==1) {
							$sqlStr="SELECT COUNT(t_no) FROM Tester WHERE t_e_no=:event_no AND t_c_no=:c_no AND t_s_no=:s_no AND t_no=:t_no AND t_is_exchange=0 AND t_is_valid=1";
							$db->query($sqlStr);
							$db->bind(':event_no', $event_no, PDO::PARAM_INT);
							$db->bind(':c_no', $c_no, PDO::PARAM_INT);
							$db->bind(':s_no', $s_no, PDO::PARAM_INT);
